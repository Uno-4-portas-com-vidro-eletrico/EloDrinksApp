...
const PageNewOrderResume = () => {
    ...
    const structureId = pack?.structure_id;
    const packageId = pack?.id;

    const { data: structureData, isLoading: isLoadingStructure } = useStructure(structureId);
    const { data: packageData, isLoading: isLoadingPackage } = usePackage(packageId);
    ...
    return (
        <View className="bg-white rounded-3xl px-2 py-2 mx-4 mt-6 shadow-md">
            ...
            <ScrollView className="h-4/6">
                {eventData && <ResumeDetails eventData={eventData} />}

                {isLoadingStructure || isLoadingPackage ? (
                    <LoadingIndicator />
                ) : (
                    <>
                        {pack && structureData && packageData && (
                            <ResumePackage
                                packData={pack}
            ...
            <View className="space-y-4 my-4">
                ...
                <AlertDialog>
                    <AlertDialogTrigger>
                        <Button label="Confirmar" className="mt-2" />
                    </AlertDialogTrigger>
                    <AlertDialogContent onConfirm={() => {
                        mutate({
                            "customer": {
